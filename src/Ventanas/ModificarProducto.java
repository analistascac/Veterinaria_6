...
public class ModificarProducto extends JFrame {
	...
	public ModificarProducto() {
		...
		cmbId.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				txtNombreCientifico.setText(prod.getNombre_cientifico());
				txtNombreVulgar.setText(prod.getNombre_vulgar());
				txtDescripcion.setText(prod.getDescripcion());
				txtPrecioCosto.setText(prod.getPrecio_costo()+"");
				txtPrecioVenta.setText(prod.getPrecio_venta()+"");
				txtLimiteInferior.setText(prod.getLimite_inferior()+"");

				listaProvedores.clear();
				for(int i = 0; i<prod.getProveedores().size()-1;i++) listaProvedores.addElement(prod.getProveedores().get(i));
			}
		});
		...
		btnAgregar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(!listaProvedores.contains(listaProveedores.getSelectedItem())){
					listaProvedores.addElement(listaProveedores.getSelectedItem());
				}
			}
		});
		...
		btnQuitar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(listaProvedores.getSize()>0 && lstProveedores.getSelectedIndex() > -1){
		...
		btnModificar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try{
					...
					prod.setIdentificador((String) cmbId.getSelectedItem());
					if(rdbtnArticulo.isSelected()){
						prod.setDescripcion(txtDescripcion.getText());
						prod.setMedida(txtMedida.getText());
					}else{
						prod.setNombre_cientifico(txtNombreCientifico.getText());
						prod.setNombre_vulgar(txtNombreVulgar.getText());
					}
					
					prod.setLimite_inferior(Integer.parseInt(txtLimiteInferior.getText()));
					prod.setPrecio_costo(Integer.parseInt(txtPrecioCosto.getText()));
					prod.setPrecio_venta(Integer.parseInt(txtPrecioVenta.getText()));
	...
	private void llenaridmascotas(){
		for(int i = 0; i<5; i++) idpmascotas.addElement("Mascota venta prueba" + i);
	}
	
	private void llenaridproductos(){
		for(int i = 0; i<5; i++) idproductos.addElement("producto venta prueba" + i);
	}
	...
	private Producto pedirProducto(String id){
		...
		prod.setIdentificador("Campo prueba");
		prod.setNombre("Campo prueba");
		prod.setMedida("Campo prueba");
		prod.setNombre_cientifico("Campo prueba");
		prod.setNombre_vulgar("Campo prueba");
		prod.setDescripcion("Campo prueba");
		prod.setPrecio_costo(12);
		prod.setPrecio_venta(12);
		prod.setLimite_inferior(5);
