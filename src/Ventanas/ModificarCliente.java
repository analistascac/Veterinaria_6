	...
	public ModificarCliente() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosed(WindowEvent arg0) {
				int x = JOptionPane.showConfirmDialog(null, "¿Realmente desea cancelar la modificación","Confirmar",JOptionPane.YES_NO_OPTION);
		...
		btnModificar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!txtNombre.getText().trim().isEmpty() &&
					...
					(rdbtnAlContado.isSelected() || rdbtnCuentaCorriente.isSelected())){
						Cliente clie = new Cliente();
						clie.setIdentificador((String)cmdId.getSelectedItem());
						clie.setNombre(txtNombre.getText());
						clie.setApellido(txtApellido.getText());
						clie.setTipo_documento((String)cmbTipoDoc.getSelectedItem());
						...
						if(rdbtnCuentaCorriente.isSelected()){
							clie.setTipo_pago("Cuenta corriente");
						}else{
							
							clie.setTipo_pago("Al contado");
						}
							
						JOptionPane.showMessageDialog(null, clie.toString());
				}else{
					JOptionPane.showMessageDialog(null, "Algún campo está incompleto","Error",JOptionPane.WARNING_MESSAGE);
				}
